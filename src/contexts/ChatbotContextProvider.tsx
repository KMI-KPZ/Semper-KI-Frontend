...
interface TopicsContextType {
  ...
  setTopics: (newTopics: Map<string,string>, newMainTopic : string, responseStructure: string, newChoices: object, addedHelp: Map<string,string>|null) => void;
...
function objectsEqual(obj1: { [key: string]: any }, obj2: { [key: string]: any }): boolean {
...
const TopicsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [topics, setTopicsState] = useState<Map<string, string>>(new Map([["semper-ki","Plattform für additive Fertigung"]]));
  ...
  const [closeChatbot, setCloseChatbot] = useState<boolean>(false);
  const [detailedHelp, setDetailedHelp] = useState<Map<string,string>>(new Map([["semper-ki","Wir vermitteln, was Sie brauchen\n" +
  ...
  const setTopics = (newTopics: Map<string,string>, newMainTopic : string, responseStructure: string, newChoices: object, addedHelp: Map<string,string>|null) => {
    setTopicsState((prevTopics) => {
      ...
      const mergedTopics = new Map([... Array.from(prevTopics.entries()), ...Array.from(newTopics.entries())]);
    ...
    setDetailedHelp((prevHelp) => {
        ...
        const mergedHelp = new Map([... Array.from(prevHelp.entries()), ...Array.from(addedHelp.entries())]);
  ...
  return (
      <TopicsContext.Provider value={{ topics, maintopic, response, choices, userChoice, setTopics,setUserChoice,closeChatbot,removeTopics,detailedHelp  }}>
        {children}
