...
import ProcessSendItem from "./components/Item";
...
const ProcessSend: React.FC<Props> = (props) => {
  const {} = props;
  const { t } = useTranslation();
  const { sendProject } = useSend();
  ...
  const processes: ProcessProps[] =
    projectQuery.data === undefined
      ...
      : projectQuery.data.processes.filter(
          (process) => process.state === ProcessState.CONTRACTOR_SELECTED
        );
  ...
  const handleOnClickButtonSend = (data: SendFormData) => {
    ...
    <div className="flex w-full flex-col items-center gap-5">
      ...
      <div className="flex w-full flex-col items-center justify-center gap-5 bg-white p-5 md:flex-row">
        <Button
          ...
          title={t("AfterProcess.Checkout.Checkout.button.send")}
        />
      </div>
