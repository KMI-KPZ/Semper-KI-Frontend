...
const ModelPreview = ({ file, className, modelName }) => {
  ...
  useEffect(() => {
    ...
    // Initialize the 3D Viewer once the component mounts
    const initViewer = async () => {
      const OV = await import("online-3d-viewer"); // Dynamically import the library

      // Clean up any existing viewer instance
      if (viewerInstance.current) {
        viewerInstance.current.Destroy(); // Clear existing model
        viewerInstance.current = null; // Reset viewer instance
      }

      // Initialize the viewer with custom parameters
      const viewer = new OV.EmbeddedViewer(viewerRef.current, {
        backgroundColor: new OV.RGBAColor(255, 255, 255, 255),
        defaultColor: new OV.RGBColor(200, 200, 200),
      });

      // Load the provided model file with modelName
      const inputFiles = [
        new OV.InputFile(modelName, OV.FileSource.Url, file),
      ];
      viewer.LoadModelFromInputFiles(inputFiles);

      viewerInstance.current = viewer; // Save the instance for cleanup
    };
    ...
    return () => {
      if (viewerInstance.current) {
        viewerInstance.current.Destroy(); // Clear the viewer content
